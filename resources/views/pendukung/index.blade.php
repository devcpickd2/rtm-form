    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h3><i class="bi bi-list-check"></i> Data Karyawan Pendukung Ready Meal</h3>
                <a href="{{ route('pendukung.create') }}" class="btn btn-success">
                    <i class="bi bi-plus-circle"></i> Tambah
                </a>
            </div>

            <div class="d-flex justify-content-between align-items-stretch mb-3">
                <a href="{{ route('pendukung.recyclebin') }}" 
        ...
        <table class="table table-striped table-bordered align-middle">
            ...
            <tbody>
                @php
                $no = ($pendukung->currentPage() - 1) * $pendukung->perPage() + 1;
                @endphp
                @forelse ($pendukung as $dep)
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{{ route('pendukung.edit', $dep->uuid) }}" class="btn btn-warning btn-sm me-1">
                            <i class="bi bi-pencil"></i> Edit
                        </a>
                        <form action="{{ route('pendukung.destroy', $dep->uuid) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="5" class="text-center">Belum ada data pendukung.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="d-flex justify-content-end">
            {{ $pendukung->withQueryString()->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
