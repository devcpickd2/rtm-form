...
<div class="container py-4">
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h3><i class="bi bi-list-check"></i> Data List Institusi Ready Meal</h3>
                <div>
                    <a href="{{ route('listinstitusi.create') }}" class="btn btn-success">
                        <i class="bi bi-plus-circle"></i> Tambah
                    </a>
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-stretch mb-3">
                <a href="{{ route('listinstitusi.recyclebin') }}" 
        ...
        <table class="table table-striped table-bordered align-middle">
            ...
            <tbody>
                @forelse ($listinstitusi as $dep)
                <tr>
                    <td>{{ \Carbon\Carbon::parse($dep->created_at)->format('d-m-Y H:i') }}</td>
                    <td>{{ $dep->nama_institusi }}</td>
                    <td class="text-center">
                        <a href="{{ route('listinstitusi.edit', $dep->uuid) }}" class="btn btn-warning btn-sm me-1">
                            <i class="bi bi-pencil"></i> Edit
                        </a>
                        <form action="{{ route('listinstitusi.destroy', $dep->uuid) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
            ...
            @empty
            <tr>
                <td colspan="4" class="text-center">Belum ada data list premix.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="d-flex justify-content-end">
        {{ $listinstitusi->links('pagination::bootstrap-5') }}
    </div>
</div>
</div>
</div>
