    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3><i class="bi bi-list-check"></i> Data Pemeriksaan Proses Thawing</h3>
                <a href="{{ route('thawing.create') }}" class="btn btn-success">
                    <i class="bi bi-plus-circle"></i> Tambah
                </a>
            </div>

            {{-- Filter dan Live Search --}}
            <form id="filterForm" method="GET" action="{{ route('thawing.index') }}" class="d-flex flex-wrap align-items-center gap-2 mb-3 p-2 border rounded bg-light shadow-sm">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered align-middle">
                    <thead class="table-primary text-center">
                        ...
                        <tr>
                            <th>Jumlah</th>
                            <th colspan="2">Kondisi Produk</th>
                            <th>Suhu Ruangan °C</th>
                            <th>Mulai Thawing</th>
                            <th>Selesai Thawing</th>
                            <th colspan="3">Kondisi Produk</th>
                            <th>Suhu Produk (5-10°C)</th>
                        </tr>
                    </thead>
                    <tbody id="tableBody">
                        @php 
                        ...
                        <tr>
                            ...
                            <td>{{ $dep->kondisi_ruangan }}</td>
                            <td>{{ $dep->jenis_produk }}</td>
                            <td>{{ $dep->kode_produksi }}</td>
                            <td>{{ $dep->jumlah }}</td>
                            <td>{{ $dep->kondisi_produk }}</td>
                            <td>Keterangan: {{ $dep->keterangan_kondisi }}</td>
                            <td>{{ $dep->suhu_ruangan ?: '-' }}</td>
                            <td>{{ \Carbon\Carbon::parse($dep->mulai_thawing)->format('H:i') }}</td>
                            <td>{{ \Carbon\Carbon::parse($dep->selesai_thawing)->format('H:i') }}</td>
                            <td>{{ $dep->kondisi_produk_setelah ?: '-' }}</td>
                            <td>Keterangan: {{ $dep->keterangan_kondisi_setelah ?: '-' }}</td>
                            <td>Jumlah: {{ $dep->jumlah_setelah ?: '-' }}</td>
                            ...
                            <td class="text-center align-middle">
                                ...
                                <td class="text-center">
                                    <a href="{{ route('thawing.edit', $dep->uuid) }}" class="btn btn-warning btn-sm me-1">
                                        <i class="bi bi-pencil"></i> Update
                                    </a>
                                    <!-- <form action="{{ route('thawing.destroy', $dep->uuid) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="19" class="text-center">Belum ada data thawing.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
