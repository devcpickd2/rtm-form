    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3><i class="bi bi-list-check"></i> Data Pemeriksaan Pemasakan dengan Steamer</h3>
                <a href="{{ route('steamer.recyclebin') }}" class="btn btn-secondary btn-sm">
                    <i class="bi bi-trash"></i> Recycle Bin
                </a>
            </div>

            {{-- Filter dan Live Search --}}
            <form id="filterForm" method="GET" action="{{ route('steamer.verification') }}" class="d-flex flex-wrap align-items-center gap-2 mb-3 p-2 border rounded bg-light shadow-sm">
                    ...
                    <tbody id="tableBody">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @php
                                $steaming = json_decode($dep->steaming, true);
                                @endphp

                                @if(!empty($steaming))
                                <a href="#" data-bs-toggle="modal" data-bs-target="#steamingModal{{ $dep->uuid }}" style="font-weight: bold; text-decoration: underline;">
                                    Hasil Steaming
                                </a>
                            ...
                            <td class="text-center align-middle">
                                ...
                                <form method="GET" action="{{ route('steamer.exportPdf') }}" target="_blank">
                                    <input type="hidden" name="uuid" value="{{ $dep->uuid }}">

                                    <button type="submit" class="btn btn-danger btn-sm fw-bold shadow-sm ms-1">
                                        <i class="bi bi-file-earmark-pdf"></i> Export PDF
                                    </button>
                                </form>
                                <form action="{{ route('steamer.destroy', $dep->uuid) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="verifyModalLabel{{ $dep->uuid }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-md">
                                    <form action="{{ route('steamer.verification.update', $dep->uuid) }}" method="POST">
                                        @csrf
                                        @method('PUT')
        ...
        @empty
        <tr>
            <td colspan="19" class="text-center">Belum ada data steaming.</td>
        </tr>
        @endforelse
    </tbody>
</table>
