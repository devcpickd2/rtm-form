...
<div class="container-fluid py-0">
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3><i class="bi bi-list-check"></i> Data Verifikasi Mesin</h3>
                <a href="{{ route('mesin.recyclebin') }}" class="btn btn-secondary btn-sm">
                    <i class="bi bi-trash"></i> Recycle Bin
                </a>
            </div>

            {{-- Filter dan Live Search --}}
            <form id="filterForm" method="GET" action="{{ route('mesin.verification') }}" class="d-flex flex-wrap align-items-center gap-2 mb-3 p-2 border rounded bg-light shadow-sm">
                ...
                <tbody id="tableBody">
                    ...
                    <tr>
                        ...
                        <td class="text-center align-middle">
                            ...
                            <a href="#" data-bs-toggle="modal" data-bs-target="#verifMesinModal{{ $dep->uuid }}" style="font-weight: bold; text-decoration: underline;">
                                verif mesin
                            </a>

                            <div class="modal fade" id="verifMesinModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="verifMesinModalLabel{{ $dep->uuid }}" aria-hidden="true">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header bg-info text-white">
                                            <h5 class="modal-title text-start" id="verifMesinModalLabel{{ $dep->uuid }}">
                                        ...
                                        <div class="modal-body p-0">
                                            <div class="table-responsive">
                                                <table class="table table-bordered table-sm mb-0">
                                                    ...
                                                    <tbody>
                                                        @foreach($verif_mesin as $index => $item)
                                                        <tr>
                                                            <td>{{ $index + 1 }}</td>
                                                            <td>{{ $item['nama_mesin'] ?? '-' }}</td>
                                                            <td>{{ $item['standar_setting'] ?? '-' }}</td>
                                                            <td>{{ $item['aktual'] ?? '-' }}</td>
                                                        </tr>
                                                        @endforeach
                                                    </tbody>
                        ...
                        <td class="text-center align-middle">{{ $dep->tindakan_perbaikan ?? '-' }}</td>
                        ...
                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="verifyModalLabel{{ $dep->uuid }}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-md">
                                <form action="{{ route('mesin.verification.update', $dep->uuid) }}" method="POST">
                                    @csrf
                                    @method('PUT')
    ...
    @empty
    <tr>
        <td colspan="25" class="text-center align-middle">Belum ada data produk.</td>
    </tr>
    @endforelse
</tbody>
</table>
...
<form method="GET" action="{{ route('mesin.exportPdf') }}">
