    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="{{ route('yoshinoya.recyclebin') }}" class="btn btn-secondary btn-sm">
                    <i class="bi bi-trash"></i> Recycle Bin
                </a>
            </div>

            {{-- Filter dan Live Search --}}
            <form id="filterForm" method="GET" action="{{ route('yoshinoya.verification') }}" class="d-flex flex-wrap align-items-center gap-2 mb-3 p-2 border rounded bg-light shadow-sm">
                ...
                <table class="table table-striped table-bordered align-middle">
                    ...
                    <tbody id="tableBody">
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">{{ $dep->saus }} | {{ $dep->kode_produksi }}</td>
                            <td class="text-center align-middle">{{ $dep->suhu_pengukuran }}</td>
                            ...
                            <td class="text-center align-middle">{{ $dep->brookfield_sebelum }}</td>
                            <td class="text-center align-middle">{{ $dep->brookfield_frozen }}</td>
                            ...
                            <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="verifyModalLabel{{ $dep->uuid }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-md">
                                    <form action="{{ route('yoshinoya.verification.update', $dep->uuid) }}" method="POST">
                                        @csrf
                                        @method('PUT')
        ...
        @empty
        <tr>
            <td colspan="19" class="text-center align-middle">Belum ada data yoshinoya.</td>
        </tr>
        @endforelse
    </tbody>
</table>
...
<form method="GET" action="{{ route('yoshinoya.exportPdf') }}">
  <div class="card shadow-sm mb-3">
      <div class="card-body d-flex align-items-end gap-2">
          ...
          {{-- Pilih Saus --}}
          <div class="col-auto">
              <label for="saus" class="col-form-label fw-semibold">Pilih Saus</label>
          </div>
          <div class="col-auto">
              <select id="saus" name="saus" class="form-control form-control-sm" required>
                  <option value="" disabled selected>Pilih saus</option>
                  <option value="teriyaki" {{ request('saus')=='teriyaki' ? 'selected' : '' }}>Teriyaki</option>
                  <option value="vegetable" {{ request('saus')=='vegetable' ? 'selected' : '' }}>Vegetable</option>
              </select>
          </div>
