    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3><i class="bi bi-list-check"></i> Data Monitoring False Rejection</h3>
                <a href="{{ route('reject.recyclebin') }}" class="btn btn-secondary btn-sm">
                    <i class="bi bi-trash"></i> Recycle Bin
                </a>
            </div>

            {{-- Filter dan Live Search --}}
            <form id="filterForm" method="GET" action="{{ route('reject.verification') }}" class="d-flex flex-wrap align-items-center gap-2 mb-3 p-2 border rounded bg-light shadow-sm">
                    ...
                    <tbody id="tableBody">
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">
                                ...
                                // Data dari database
                                $rejectData = [
                                'jumlah_tidak_lolos' => $dep->jumlah_tidak_lolos,
                                'jumlah_kontaminan'  => $dep->jumlah_kontaminan,
                                'jenis_kontaminan'   => $dep->jenis_kontaminan,
                                'posisi_kontaminan'  => $dep->posisi_kontaminan,
                                'false_rejection'    => $dep->false_rejection,
                                ];
                                ...
                                data-bs-target="#detailRejectModal{{ $dep->uuid }}" 
                                style="font-weight:bold; text-decoration:underline;">
                                Detail
                            </a>

                            <!-- Modal -->
                            <div class="modal fade" id="detailRejectModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="detailRejectModalLabel{{ $dep->uuid }}" aria-hidden="true">
                                <div class="modal-dialog modal-md">
                                    <div class="modal-content">
                                        <div class="modal-header bg-info text-white">
                                            <h5 class="modal-title" id="detailRejectModalLabel{{ $dep->uuid }}">
                                                ...
                                                <table class="table table-bordered table-sm mb-0">
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td>{{ $dep->jumlah_tidak_lolos ?? '-' }}</td>
                                                        </tr>
                                                        <tr>
                                                            <td>Jumlah Kontaminan</td>
                                                            <td>{{ $dep->jumlah_kontaminan ?? '-' }}</td>
                                                        </tr>
                                                        <tr>
                                                            <td>Jenis Kontaminan</td>
                                                            <td>{{ $dep->jenis_kontaminan ?? '-' }}</td>
                                                        </tr>
                                                        <tr>
                                                            <td>Posisi Kontaminan</td>
                                                            <td>{{ $dep->posisi_kontaminan ?? '-' }}</td>
                                                        </tr>
                                                        <tr>
                                                            <td>False Rejection</td>
                                                            <td>{{ $dep->false_rejection ?? '-' }} / {{ $dep->jumlah_tidak_lolos ?? '-' }}</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                        ...
                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="verifyModalLabel{{ $dep->uuid }}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-md">
                                <form action="{{ route('reject.verification.update', $dep->uuid) }}" method="POST">
                                    @csrf
                                    @method('PUT')
...
<form method="GET" action="{{ route('reject.exportPdf') }}">
