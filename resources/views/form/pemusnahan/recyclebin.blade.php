...
<div class="container-fluid py-4">
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h3><i class="bi bi-trash"></i> Recycle Bin Pemusnahan Barang/Produk</h3>
                <a href="{{ route('pemusnahan.verification') }}" class="btn btn-primary">
                ...
                <table class="table table-striped table-bordered table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">{{ $item->analisis ?: '-' }}</td>
                            <td class="text-center align-middle">{{ $item->keterangan ?: '-' }}</td>
                            <td class="text-center align-middle">{{ $item->username }}</td>
                            <td class="text-center align-middle">{{ \Carbon\Carbon::parse($item->deleted_at)->format('d-m-Y H:i') }}</td>

                            <td class="text-center align-middle">
                                <form action="{{ route('pemusnahan.restore', $item->uuid) }}" method="POST" class="d-inline">
                                    @csrf
                                ...
                                <form action="{{ route('pemusnahan.deletePermanent', $item->uuid) }}" 
    ...
    <div class="d-flex justify-content-end mt-2">
        {{ $pemusnahan->links('pagination::bootstrap-5') }}
    </div>
</div>
</div>
