    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3><i class="bi bi-list-check"></i> Data Pemeriksaan X RAY</h3>
                <a href="{{ route('xray.recyclebin') }}" class="btn btn-secondary btn-sm">
                    <i class="bi bi-trash"></i> Recycle Bin
                </a>
            </div>

            {{-- Filter dan Live Search --}}
            <form id="filterForm" method="GET" action="{{ route('xray.verification') }}" class="d-flex flex-wrap align-items-center gap-2 mb-3 p-2 border rounded bg-light shadow-sm">
                        ...
                        <tr>
                            ...
                            <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="verifyModalLabel{{ $dep->uuid }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-md">
                                    <form action="{{ route('xray.verification.update', $dep->uuid) }}" method="POST">
                                        @csrf
                                        @method('PUT')
...
<form method="GET" action="{{ route('xray.exportPdf') }}">
