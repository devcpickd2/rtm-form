    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h3><i class="bi bi-list-check"></i> Data List Premix Ready Meal</h3>
                <div>
                    <a href="{{ route('listpremix.create') }}" class="btn btn-success">
                        <i class="bi bi-plus-circle"></i> Tambah
                    </a>
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-stretch mb-3">
                <a href="{{ route('listpremix.recyclebin') }}" 
        ...
        <table class="table table-striped table-bordered align-middle">
            <thead class="table-primary text-center">
                <tr>
                    ...
                    <th>Nama Premix</th>
                    <th>Alergen/Non Alergen</th>
                    <th style="width: 20%;">Action</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($listpremix as $dep)
                <tr>
                    <td>{{ \Carbon\Carbon::parse($dep->created_at)->format('d-m-Y H:i') }}</td>
                    <td>{{ $dep->nama_premix }}</td>
                    <td>{{ $dep->alergen }}</td>
                    <td class="text-center">
                        <a href="{{ route('listpremix.edit', $dep->uuid) }}" class="btn btn-warning btn-sm me-1">
                            <i class="bi bi-pencil"></i> Edit
                        </a>
                        <form action="{{ route('listpremix.destroy', $dep->uuid) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
